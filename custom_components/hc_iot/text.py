"""Text platform for Hc IoT integration."""
...
async def async_setup_entry(
    ...
    if manager is not None:
        entities = []
        devices: list[ManagerDevice] = await manager.getTextList()
        for device in devices:
            entities.extend(
                HcTextEntity(device, model, manager) for model in device.models
            )
        manager.text_list = entities
        async_add_entities(entities)
    return True


class HcTextEntity(ManagerDevice, TextEntity):
    """Hc Text Entity."""
    ...
    async def async_set_value(self, value: str) -> None:
        ...
        if value is not None:
            await self.manager.instruction(
                self.device.device_id, self.model.type, value
            )
